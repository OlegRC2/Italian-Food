...
*/
var rqAnFr = __webpack_require__(/*! raf */ "./node_modules/raf/index.js");
...
function form() {
  var forms = document.getElementsByTagName('form'); // берем все формы
  ...
  forms.forEach(function (form) {
    ...
    form.addEventListener('submit', function (e) {
      // навешиваем на каждую форму обработчик срабатывающий при подтверждении формы
      e.preventDefault(); // отменяем стандартное поведение браузера (перезагрузку страницы)

      var statusMessageSpnr = document.createElement('img'),
          // создаем новый элемент на странице для показа его пользователю в котором будет спиннер
      ...
      var formData = new FormData(form); // создаем переменную по классу FormData. Этот класс собирает данные, которые отправить надо. У инпутов в формах должен быть обязательно быть атрибут "name", без него работать не будет

      Object(_services_requests__WEBPACK_IMPORTED_MODULE_5__["postData"])('./server.php', formData) // отправялем запрос
      .then(function (data) {
        // получаем ответ с сервера, data - это данные с сервера
        ...
        // блок в котором выполняются действия независимо от того что было выполнено, then или catch
        form.reset(); // сбросить данные в форме
      });

      function showThanksModal(message) {
        // функция для создания сообщения полсе отправки формы
        var thanksModal = document.createElement('div'); // создаем элемент
...
/* harmony default export */ __webpack_exports__["default"] = (moreText);
...
function search(inputSelector) {
  var context = document.body,
      // получаем весь сайт для поиска по нему
  input = document.querySelector(inputSelector);
  var instance = new Mark(context); // активируем библиотеку mark, ищем по всему сайту, взято из документации
...
function slider(_ref) {
  ...
  var pbs = 0; // среднее расстояние между слайдами (если слайды имеют разную ширину и выровнены флексом)

  function placeBetweenSlides(field, slides) {
    // функция для рассчета среднего расстояния между слайдами (если слайды имеют разную ширину и выровнены флексом)
    var fieldWidth = deleteNotDigits(window.getComputedStyle(field).width); // получаем ширину полосы со всеми слайдами

    var allSlideWidth = 0; // ширина всех слайдов вместе взятых
  ...
  var offset = 0,
      // отступ влево или вправо при прокрутке слайдера
  numHideSlideNext = 0,
      // счетчик скрытых слайдов вперед
  numHideSlidePrev = 0;
  ...
  function slideNext(numSlides, slide) {
    // функция для перелистывания слайдера вперед. Аргументы: numSlides - кол-во отображаемых одновременно элементов, slide - псевдомассив со всеми слайдами 
    var slideWidth = 0; // ширина следующего слайда

    if (numHideSlideNext == 0 || numHideSlideNext == numSlides) {
      // слайдер в начальном положении. Либо 0 - только открыли сайт, либо numSlides - уже тыкали "назад"
    ...
    } else if (numHideSlideNext > 0 && numHideSlideNext != slide.length) {
      // слайдер в промежуточном положении (не начальное и есть еще скрытые следующие слайды)
      ...
      numHideSlidePrev++; // добавляем 1 слайд к счетчику слайдов, который можно вернуть нажав "назад"
    } else if (numHideSlideNext == slide.length) {
      // слайдер в конечном положении
  ...
  function slidePrev(numSlides, slide) {
    // функция для перелистывания слайдера назад. Аргументы: numSlides - кол-во отображаемых одновременно элементов, slide - псевдомассив со всеми слайдами 
    var slideWidth = 0; // ширина предыдущего слайда
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _lib_parallax__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./lib/parallax */ "./src/js/lib/parallax.js");
...
/* harmony import */ var _modules_moreText__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/moreText */ "./src/js/modules/moreText.js");
/* harmony import */ var _modules_form__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/form */ "./src/js/modules/form.js");
 // подключение библиотеки параллакса
...
window.addEventListener('DOMContentLoaded', function () {
  var scene = document.getElementById('scene'); // активация библиотеки параллакса

  var parallaxInstance = new _lib_parallax__WEBPACK_IMPORTED_MODULE_0__["default"](scene); // взято из документации
  ...
  new WOW().init();
  Object(_modules_search__WEBPACK_IMPORTED_MODULE_1__["default"])('.header__search'); // поиск по сайту через библиотеку mark

  window.addEventListener('load', function () {
    // чтобы слайдер корректно отрабатывал ждем пока загрузится вся страница (все стили, картинки и т.п.)
    Object(_modules_slider__WEBPACK_IMPORTED_MODULE_2__["default"])({
      nextArrow: '.slider-block__next',
      // стрелка "следующий слайд"
      prevArrow: '.slider-block__prev',
      // стрелка "предыдущий слайд"
      slide: '.slider-block__item',
      // сам слайд
      wrapper: '.slider-block__slider-window',
      // обертка всего слайдера
      field: '.slider-block__slider-wrapper' // длинный блок со всеми слайдами

    });
  });
  Object(_modules_moreText__WEBPACK_IMPORTED_MODULE_3__["default"])('.more-text__btn', '.more-text__hide-text', '.more-text');
